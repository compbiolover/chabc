...
processed_dataframes <- lapply(all_dfs, function(df) {
  process_dataframe_map(df, errors_summary = df_errors_summary, latency_summary = df_latency_summary, k = 2, print_results = FALSE, plot_silhouette = TRUE)
})
...
# Change line to access write folder
all_files <- list.files(path = "Outputs/Tables/stats_tables/kmeans/", pattern = "By Average Errors")
files_list <- list()
for (f in all_files) {
  current_file <- read.csv(file = paste0("Outputs/Tables/stats_tables/kmeans/", f), row.names = 1)
  files_list[[f]] <- current_file
}
...
cor_estimates <- cor_estimates %>%
  select(-method) %>%
  mutate(row_name = paste(subregion, kmeans_label, sep = "_"))

# Now moving our finished calculation matrix into our heatmap matrix
for (s in unique(cor_estimates$subregion)) {
  current_sub <- filter(cor_estimates, subregion == s)
  for (l in current_sub$kmeans_label) {
    current_brain_region <- filter(current_sub, kmeans_label == l)
...
```{r saving finished heatmaps}
# PNG file format
png(file = paste0("Outputs/Plots/finished_plots/complex_heatmap_combined_cohorts_by_subregion_", analysis_type, "_", outcome_type, ".png"), units = "in", width = 6, height = 6, res = 600)
p1 <- draw(p1, height = unit(1, "cm") * 6)
draw(p1, height = unit(1, "cm") * 6)
dev.off()
...
# SVG file format
svg(file = paste0("Outputs/Plots/finished_plots/complex_heatmap_combined_cohorts_by_subregion_", analysis_type, "_", outcome_type, ".svg"), width = 6, height = 6)
p1 <- draw(p1, height = unit(1, "cm") * 6)
draw(p1, height = unit(1, "cm") * 6)
dev.off()
```
...
process_dataframe_map_all_subs <- function(df, errors_summary = NULL, latency_summary = NULL, k = 2, print_results = TRUE, plot_silhouette = TRUE) {
  ...
  if (!check_unique_value(matrix(df$reduction))) {
    ...
    df <- df %>%
      mutate(
        average_errors = rep(errors_summary, nrow(df)),
        average_latency = rep(latency_summary, nrow(df))
      )
...
processed_dataframe <- process_dataframe_map_all_subs(all_sub_df, errors_summary = avg_errors, latency_summary = avg_latency, k = 2, print_results = FALSE, plot_silhouette = FALSE)

# Processing of finished dataframe
processed_dataframe_filtered <- filter(processed_dataframe, kmeans < 10)
...
# Now running it to see what all cohorts and subregions together look like
test <- plot_correlation_all_subregions(
  data = processed_dataframe_filtered,
  x_var = kmeans,
  y_var = average_errors,
  calculate_correlation = TRUE,
  correlation_method = "spearman",
