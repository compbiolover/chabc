...
plot_correlation_by_subregion <- function(data, x_var, y_var, calculate_correlation = TRUE, correlation_method = "spearman", plot_color = cohort, plot_shape = kmeans_label, point_size = 2, x_label, y_label, plot_title, subregion = NULL) {
  ...
  if (deparse(substitute(x_var)) == "kmeans") {
    ...
    all_map_above_rost <- data_rost %>%
      filter(map > mean(map, na.rm = TRUE))

    all_map_below_rost <- data_rost %>%
      filter(map < mean(map, na.rm = TRUE))

    all_map_above_caud <- data_caud %>%
      filter(map > mean(map, na.rm = TRUE))

    all_map_below_caud <- data_caud %>%
      filter(map < mean(map, na.rm = TRUE))

    all_rost_strip_df <- bind_rows(all_map_above_rost, all_map_below_rost)
    all_caud_strip_df <- bind_rows(all_map_above_caud, all_map_below_caud)
    all_sub_plot_df <- bind_rows(all_rost_strip_df, all_caud_strip_df)

    # rost_p <- ggplot(data = all_rost_strip_df, aes(x= mean_label, y = map, color = cohort))+
    #   geom_jitter()+
    ...
    #   ylab("Map #")
    #
    # caud_p <- ggplot(data = all_caud_strip_df, aes(x= mean_label, y = map, color = cohort))+
    #   geom_jitter()+
    #   theme(panel.background = element_blank(),
    #         axis.title = element_text(size = 10),
    #         axis.text = element_text(size = 9))+
    ...
    sub_strip_p <- ggplot(data = all_sub_plot_df, aes(x = mean_label, y = map, color = cohort)) +
      geom_jitter() +
      theme(
        panel.background = element_blank(),
        axis.title = element_text(size = 10),
    ...
    sub_strip_p <- sub_strip_p + stat_summary(
...
all_files <- list.files(path = "Outputs/Tables/stats_tables/", pattern = "By Average Latency")
files_list <- list()
for (f in all_files) {
  current_file <- read.csv(file = paste0("Outputs/Tables/stats_tables/", f), row.names = 1)
  files_list[[f]] <- current_file
}
...
# Define column annotation for strip chart of the heatmap
strip_p <- ggplot(aes())
...
# Function to fit a linear regression model and calculate RMSE
...
# Function to process the dataframe and build linear regression models
process_and_build_linear_models <- function(dataframe) {
  processed_dataframe <- dataframe |>
    drop_na()

  # Build and evaluate linear regression models
  kmeans_model <- fit_linear_model(processed_dataframe, average_latency ~ kmeans_label)
  mean_model <- fit_linear_model(processed_dataframe, average_latency ~ mean_label)
  median_model <- fit_linear_model(processed_dataframe, average_latency ~ median_label)
  reduction_model <- fit_linear_model(processed_dataframe, average_latency ~ reduction)
...
# Using the functions
linear_models <- process_and_build_linear_models(processed_dataframe2)

# Access the models and their RMSEs
kmeans_model <- linear_models$kmeans_model
kmeans_rmse <- kmeans_model$rmse
print(paste("K-means RMSE:", kmeans_rmse))
summary(kmeans_model$model) # Print model summary

mean_model <- linear_models$mean_model
mean_rmse <- mean_model$rmse
print(paste("Mean RMSE:", mean_rmse))
summary(mean_model$model) # Print model summary

median_model <- linear_models$median_model
median_rmse <- median_model$rmse
print(paste("Median RMSE:", median_rmse))
summary(median_model$model) # Print model summary

reduction_model <- linear_models$reduction_model
reduction_rmse <- reduction_model$rmse
print(paste("Reduction RMSE:", reduction_rmse))
summary(reduction_model$model) # Print model summary
```
