...
# Loading packages
```{r loading packcages}
pacman::p_load(broom, cluster, ComplexHeatmap, ggforce, ggpubr, gridExtra, reshape2, rstatix, tidyverse, viridis)
```
...
# Functions to calculate very rostral and very caudal labels from all subregions and cohorts together in one dataframe
# Function to extract relevant data to get ready for model format
extract_columns <- function(data, file_name = "ssp_bfd") {
  # Initialize an empty list to store the extracted columns
  extracted_columns <- list()
...
process_dataframe_map_all_subs <- function(df, errors_summary = NULL, latency_summary = NULL, k = 2, print_results = TRUE, plot_silhouette = TRUE) {
  # Clustering Approach (K-means)
  k <- k
  if (!check_unique_value(matrix(df$reduction))) {
    # Mean Approach
...
process_dataframe_map_all_subs_dynamic_kmeans <- function(df, errors_summary = NULL, latency_summary = NULL, k = 2, print_results = TRUE, plot_silhouette = TRUE) {
  ...
  if (!check_unique_value(matrix(df$reduction))) {
    ...
    if (plot_silhouette) {
      silhouette_scores <- silhouette(clustering$cluster, dist(df$map))
      png(filename = paste0("Outputs/Plots/silhouette_plots/silhouette_plot_", k, "_clusters_all_subregions_all_cohorts_map.png"), width = 8, height = 8, units = "in", res = 600, bg = "white")
      plot(silhouette_scores, main = paste0("K-means Clustering | Clusters: ", k), col = "skyblue")
      dev.off()
    }
...
# Dynamic k-means
processed_dataframe2 <- process_dataframe_map_all_subs_dynamic_kmeans(all_sub_df, errors_summary = df_errors_summary, latency_summary = df_latency_summary, k = 3, print_results = FALSE, plot_silhouette = TRUE)
...
# Now binning by 0.1 mm increments as we have too many coefficients
# Function to generate bins and labels
generate_bins_labels <- function(interval, min_value = 0, max_value = 1) {
  # Generate the sequence of breaks based on the interval
  bins <- seq(min_value, max_value, by = interval)

  # Generate the labels based on the breaks
  labels <- paste0(format(bins[-length(bins)], nsmall = 2), "-", format(bins[-1], nsmall = 2))

  return(list(bins = bins, labels = labels))
}


interval <- 0.1
generated_bins_labels <- generate_bins_labels(interval)
bins <- generated_bins_labels$bins
labels <- generated_bins_labels$labels
...
processed_dataframe2 <- processed_dataframe2 |>
  drop_na()
mod <- lm(average_latency ~ kmeans_label, data = processed_dataframe2)
summary(mod)
...
mod_analysis_by_sub <- function(sub = NULL, data = NULL){
  current_sub <- filter(.data = {{data}}, subregion == {{sub}})
  mod <- lm(average_latency ~ kmeans_label, data = current_sub)
  print(summary(mod))
  print(paste("RMSE:", sqrt(mean(mod$residuals^2))))
}
...
# By subregion
for (s in unique(processed_dataframe2$subregion)){
  print(paste("Subregion: ", s))
  mod_analysis_by_sub(sub = s, data = processed_dataframe2)
}


# By cohort
mod_analysis_by_coh <- function(coh = NULL, data = NULL){
  current_sub <- filter(.data = {{data}}, cohort == {{coh}})
  mod <- lm(average_latency ~ kmeans_label, data = current_sub)
  print(summary(mod))
  print(paste("RMSE:", sqrt(mean(mod$residuals^2))))
}
...
processed_dataframe2 <- processed_dataframe2|>
  filter(cohort != "#071022")

for (c in unique(processed_dataframe2$cohort)){
  print(paste("Cohort: ", c))
  mod_analysis_by_coh(coh = c, data = processed_dataframe2)
}
```
...
sum(!complete.cases(processed_dataframe2))
rf_processed_dataframe2 <- processed_dataframe2 |>
  drop_na()
rf_mod <- randomForest(average_latency ~ kmeans_label, data = rf_processed_dataframe2)
rf_mod
...
# Tuning RF model
model_tuned <- tuneRF(
               x=as.data.frame(rf_processed_dataframe2$kmeans_label), #define predictor variables
               y=rf_processed_dataframe2$average_latency, #define response variable
               ntreeTry=500,
               improve=0.01,
