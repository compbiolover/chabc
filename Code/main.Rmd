...
set.seed(1715030415)
rf_mod_kmeans <- randomForest(average_latency ~ kmeans_label, data = rf_processed_dataframe2)
rf_mod_kmeans

# find number of trees that produce lowest test MSE
which.min(rf_mod_kmeans$mse)

# find RMSE of best model
sqrt(rf_mod_kmeans$mse[which.min(rf_mod_kmeans$mse)]) # 0.1394328

# Median RF mod
rf_mod_median <- randomForest(average_latency ~ median_label, data = rf_processed_dataframe2)
rf_mod_median

# find number of trees that produce lowest test MSE
which.min(rf_mod_median$mse)

# find RMSE of best model
sqrt(rf_mod_median$mse[which.min(rf_mod_median$mse)]) # RMSE: 0.1427178


# Mean RF mod
rf_mod_mean <- randomForest(average_latency ~ mean_label, data = rf_processed_dataframe2)
rf_mod_mean

# find number of trees that produce lowest test MSE
which.min(rf_mod_mean$mse)

# find RMSE of best model
sqrt(rf_mod_mean$mse[which.min(rf_mod_mean$mse)]) # RMSE: 0.1432065
```
