...
# Function to plot and calculate correlations for all cohorts withiin a subregion
plot_correlation_by_subregion <- function(data, x_var, y_var, calculate_correlation = TRUE, correlation_method = "spearman", plot_color = cohort, plot_shape = kmeans_label, point_size = 2, x_label, y_label, plot_title, subregion = NULL) {
  ...
  p <- ggplot(data, aes(x = {{ x_var }}, y = {{ y_var }}, shape = {{ plot_shape }}, size = point_size)) +
    ...
    guides(size = "none") +
    facet_row(~kmeans_label, space = "free", scales = "free_x")

  ggsave(plot = p, filename = paste(tolower(plot_title), "_", correlation_method, "_all_cohorts_map_based_separation.png"), device = "png", width = 8, height = 8, units = "in", dpi = 600, bg = "white", path = "Outputs/Plots/exploratory_plots/")
}



all_processed_dfs_filtered_subregion <- all_processed_dfs_filtered %>% filter(subregion == "ssp_bfd")
